...
const GENERIC_REQUEST_MESSAGE = '입력하신 이메일로 인증 코드를 발송했습니다. 이메일을 확인해 주세요.'
...
export async function requestPasswordReset(
  ...
  try {
    ...
    return { success: false, message: '요청 처리 중 오류가 발생했습니다. 잠시 후 다시 시도해 주세요.' }
  }
}
...
export async function verifyResetCode(
  ...
  try {
    const user = await prisma.user.findUnique({ where: { email } })
    if (!user) {
      return { success: false, message: GENERIC_VERIFY_ERROR, fieldErrors: { code: GENERIC_VERIFY_ERROR } }
    }
    ...
    return { success: false, message: '인증 처리 중 오류가 발생했습니다. 잠시 후 다시 시도해 주세요.' }
  }
}
...
export async function resetPassword(
  ...
  try {
    ...
    return { success: true, message: '비밀번호가 재설정되었습니다. 새 비밀번호로 다시 로그인해 주세요.' }
  } catch (error) {
    console.error('[PasswordReset] Reset failed:', error)
    return { success: false, message: '비밀번호 재설정 중 오류가 발생했습니다. 잠시 후 다시 시도해 주세요.' }
  }
}
